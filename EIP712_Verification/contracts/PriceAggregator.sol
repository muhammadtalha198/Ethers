...
contract PriceAggregator {
    ...
    // PriceBatch(bytes32 tokensHash,bytes32 pricesHash,uint64 validUntil,uint256 nonce)
    bytes32 private constant PRICE_BATCH_TYPEHASH = keccak256("PriceBatch(bytes32 tokensHash,bytes32 pricesHash,uint64 validUntil,uint256 nonce)");
    ...
    /// @param tokens Token addresses to update.
    /// @param prices Scaled integer prices (must match configured decimals per token).
    ...
    function updatePricesSigned(
        ...
        // Mark nonce as used before state updates to block replays in case of reentrancy (no external calls here though)
